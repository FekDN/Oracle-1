"""
Oracle-1 · Production Training Script  (v2 — Two-Phase Architecture)
======================================================================

КЛЮЧЕВОЕ ИЗМЕНЕНИЕ v2
----------------------
Предыдущая версия смешивала два принципиально разных процесса в одном
цикле "эпох":
  (старый цикл)  for epoch in range(N):
                     oracle_system_epoch()   # символическая генерация графа
                     train_one_epoch()       # обучение нейросети на СТАТИЧНОМ датасете

Это было логически некорректно:
  1. Нейросеть никогда не видела граф, который строила символическая часть —
     DataLoader создавался один раз до цикла и не перестраивался.
  2. Механизм внимания (GATv2) обучался на снапшоте инициализации,
     а не на реальном эволюционирующем графе.
  3. Cosine Annealing был привязан к числу эпох, не имеющих смысла
     как единица нейросетевого обучения.

НОВАЯ АРХИТЕКТУРА — два явных этапа:
─────────────────────────────────────────────────────────────────────
